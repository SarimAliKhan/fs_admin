    ...
    import {getAttributes,getImages,getAttributesDetails,getCategory,getCollections} from '../../Database/GetMethods'
    ...
    const UpdateAddProduct = () => {
        ...
        const [detailOfTheProduct,setDetailOfTheProduct] = useState(()=>EditorState.createEmpty());

        useEffect( ()=>{
            Promise.all([getAttributes(),getAttributesDetails(),getCategory(),getCollections(),getImages()])
            .then(values=>{
                ...
                return(
                    <Grid style={{padding:'3px'}} container >
                            ...
                            <Grid container>
                                {   attribute.map(item=>(
                                        <Grid key={item.id} item md={2}>
                                            ...
                                            {
                                                <Input type="select"  defaultValue=''  id={`attribute${id}${item.id}`}>
                                                    {
                                                        attributesDetailsList.map(attributeDetail=>(
                                                            attributeDetail.attId==item.id ?
                                                                <option key={attributeDetail.attDetailsId} value={attributeDetail.attDetailsId}>
                                                                    {
                        ...
                        <Grid style={{padding:'3px'}} item sm={12} xs={12} md={2}>
                            ...
                            <TextField  label="sku" id={`sku${id}`} variant="outlined" fullWidth type="text"/>
                        </Grid>
                        <Grid style={{padding:'3px'}} item sm={12} xs={12} md={2}>
                            <label htmlFor={`quantity${id}`}>Quantity</label>
                            <TextField   label="quantity" required id={`quantity${id}`} variant="outlined" fullWidth type="number"/>
                        </Grid>
                        <Grid style={{padding:'3px'}} item md={2} sm={12} xs={12}>
                            <label htmlFor={`costprice${id}`}>Cost Price</label>
                            <TextField   label="cost price" required id={`costprice${id}`} variant="outlined" fullWidth type="number"/>
                        </Grid>
                        <Grid style={{padding:'3px'}} item md={2} sm={12} xs={12}>
                            <label htmlFor={`saleprice${id}`}>Sale Price</label>
                            <TextField   label="sale price" id={`saleprice${id}`} required variant="outlined" fullWidth type="number"/>
                        </Grid>
                        <Grid style={{padding:'3px'}} item md={2} sm={12} xs={12}>
                            <label htmlFor={`discount${id}`}>Discount</label>
                            <TextField   label="discount in %" id={`discount${id}`} variant="outlined" fullWidth type="number"/>
                        </Grid>
                        <Grid style={{padding:'3px'}} md={2} sm={12} xs={12}>
                            <label htmlFor={`weight${id}`}>Weight</label>
                            <TextField   label="weight in gram" id={`weight${id}`} required variant="outlined" fullWidth type="number"/>
                        </Grid>
                        <Grid item id={`showImages${id}`} md={12} sm={12} xs={12}>
                            {
                                image.map(e=>(
                                    <img style={{height:'150px',width:'150px',margin:'5px',borderRadius:'50%'}} src={e.src} />
                                ))
                            }
                        </Grid>
                        <br/>    
                        <Grid item md={12} sm={12} xs={12}>
                            <Button style={{margin:'5px'}} onClick={()=>setOpen(true)} variant="contained" color="primary">
                                choose Images
                            </Button>
                            {
                                image.length > 2 ?
                                <Button onClick={handleFinalImage} style={{margin:'5px'}} color="primary" variant="contained">
                                    Upload Images
                                </Button>
                                :
                                null
        ...
        const SubmitData = () => {
            ...
            const finalObj = {
                ProductInfo : {
                    ...
                    shipping_details:draftToHtml(convertToRaw(detailOfTheProduct.getCurrentContent())),
                    usp:productUsp,
            ...
            <Paper style={{padding:'30px'}}>  
                <Grid container justify="center">
                    <Grid item md={12}>
                        ...
                        <TextField required value={productName} onChange={handleProductName}  variant="outlined" id="productname" helperText="Product name in any language" type="text" fullWidth  label="Enter the product name"/>
                        <label htmlFor="usp">product USP</label>
                        <TextField required value={productUsp} onChange={handleProductUsp}  variant="outlined" id="usp" helperText="One line usp" type="text" fullWidth  label="Enter one line USP"/>
                            ...
                            <Grid style={{padding:'5px'}} xs={12} sm={12} item md={2}>
                                <h4>Categories</h4>
                                    ...
                                    <MenuItem value='' disabled>Please Selct</MenuItem>
                                    {
                                        ...
                                        <MenuItem key={category.catId} id={category.catId} value={category.catId}>
                                        {
                                            category.catName
                                        }
                                        </MenuItem>
                        ...
                        <Grid container>
                        {
                            attributesList.map(att=>(
                                ...
                                <label htmlFor={att.attributeName} >{att.attributeName}</label>
                                    <Checkbox
                                        id={att.attId}
                                        onChange={(e)=>handleCheckbox(e,att.attributeName,att.attId)}
                                    />  
                                </Grid>
                            ))
                        }
                        ...
                        <Grid id="attribute" container>    
                            ...
                            { attribute.length>0 ? uiCount.map(item=>(
                                <div key={item.id}>
                                    {ui(item)}
                                </div>
                            ))
                            ...
                            {
                                attribute.length > 0 ?
                                <Button onClick={handleReRender}  style={{margin:'5px 0px'}} variant="contained"  color="primary">
                                    Add Data
                                </Button>
                                :
                                null
                        ...
                        <Grid container  >
                            <Grid item={2} style={{marginRight:'5px'}}>
                                <Button color="primary"  onClick={handleIsRemoved} variant="contained">Is Removed</Button>
                            </Grid>
                            <Grid item={2}>
                                <Button color="primary" onClick={handleDeactivated} variant="contained">Deactivated</Button>
                            </Grid>
