    ...
    for(let i = 0; i < res.length; i ++){
      layout.push(
        <tr key={i}>
          ...
          <td>{res[i].userId}</td>
          <td>{res[i].patientAge}</td>
          <td>{res[i].contactNumber}</td>
          <td>{res[i].city}</td>
          <td>{res[i].bloodGroup}</td>
          <td>{res[i].occupationPatient}</td>
        ...
        <div className="animated fadeIn">
          <Row>
            <Col xl={12}>
              <Card>
                <CardHeader>
                  <i className="fa fa-align-justify"></i> Plasma Request List <small className="text-muted"></small>
